...
must_haves:
  truths:
    ...
    - "Concurrent requests for same user do not create duplicates"
  artifacts:
...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <how-to-verify>
    ...
    **Step 5: Verify concurrent upsert prevents duplicates**
    1. Start the API: `npm run dev --filter=@populatte/api`
    2. Open a terminal and run this test script (requires `curl` and `jq`):
       ```bash
       # Simulate 3 concurrent requests with same clerkId
       # Replace YOUR_JWT_TOKEN with a valid token from Step 4
       TOKEN="YOUR_JWT_TOKEN"
       CLERK_ID="test_concurrent_$(date +%s)"

       # Fire 3 requests simultaneously
       curl -X POST http://localhost:3001/api/users/sync \
         -H "Authorization: Bearer $TOKEN" \
         -H "Content-Type: application/json" \
         -d "{\"clerkId\":\"$CLERK_ID\",\"email\":\"test@example.com\"}" &
       curl -X POST http://localhost:3001/api/users/sync \
         -H "Authorization: Bearer $TOKEN" \
         -H "Content-Type: application/json" \
         -d "{\"clerkId\":\"$CLERK_ID\",\"email\":\"test@example.com\"}" &
       ...
       wait
       ```
    3. Check database: `SELECT COUNT(*) FROM users WHERE clerk_id = '$CLERK_ID'`
    4. Should return exactly 1 row (not 3) - the upsert mechanism prevents duplicates

    **Note:** If the /api/users/sync endpoint doesn't exist yet (will be created in Phase 02), this step can be verified by:
    - Confirming the `onConflictDoUpdate` pattern exists in the repository code
    - The PostgreSQL `INSERT ON CONFLICT DO UPDATE` is inherently atomic and race-condition safe
    - This verification can be deferred to Phase 02 integration testing

    **Expected outcomes:**
    - Database has new columns and partial unique index
    - App fails fast on missing env vars
    - JWT contains user profile claims
    - Concurrent requests do not create duplicate user records
  </how-to-verify>
...
3. **CFG-03:** Concurrent requests for same user do not create duplicates
   - Verified by upsert mechanism using `INSERT ON CONFLICT DO UPDATE`
   - The partial unique index on clerkId ensures atomic conflict detection
   - Concurrent requests result in single database record

4. **Bonus:** Environment validation fails fast on missing required vars
