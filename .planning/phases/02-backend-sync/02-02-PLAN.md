...
must_haves:
  ...
  key_links:
    - from: "apps/api/src/infrastructure/auth/guards/clerk-auth.guard.ts"
      ...
      via: "DI constructor injection for direct upsert (not via SyncUserUseCase)"
...
Architecture note: The guard calls `userRepository.upsert()` **directly** -- it does NOT use SyncUserUseCase. The guard has its own compare-first optimization (fetch stored user, compare profile fields, skip write if unchanged) which is fundamentally different from the use case's simple unconditional upsert. SyncUserUseCase is scoped to the webhook flow only (Plan 02-01).
...
**Important: The guard calls `userRepository.upsert()` directly -- it does NOT use SyncUserUseCase.** The guard's compare-first logic (fetch, compare, conditionally write) is different from the use case's unconditional upsert. SyncUserUseCase is for the webhook flow only.
...
Verify the guard does NOT import or use SyncUserUseCase -- it calls userRepository.upsert() directly.
  </verify>
  <done>
ClerkAuthGuard performs compare-first sync: extracts JWT claims, fetches stored user, compares profile fields, upserts only on mismatch via direct userRepository call (not SyncUserUseCase). Sync failures return 503 with generic message. Health indicator tracks success/failure rate. Full User entity attached to request.
  </done>
</task>
...
5. ClerkAuthGuard calls userRepository.upsert() directly (NOT SyncUserUseCase)
6. ClerkAuthGuard throws ServiceUnavailableException (503) on sync failure, not UnauthorizedException (401)
7. CurrentUser decorator returns `User` entity
8. UserController.getMe() has no database call -- returns User from decorator
9. SyncFailureIndicator.recordSyncAttempt() called on both success and failure paths
</verification>
...
- Guard uses userRepository.upsert() directly, not SyncUserUseCase (compare-first pattern)
