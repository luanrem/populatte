...
must_haves:
  truths:
    ...
    - "Pagination defaults to limit=50, offset=0, max limit=100"
...
@apps/api/src/core/repositories/row.repository.ts (findByBatchIdPaginated exists, needs countByBatchId)
...
<task type="auto">
  <name>Task 1: Core layer — countByBatchId method and three use cases</name>
  <files>
    apps/api/src/core/repositories/row.repository.ts
    apps/api/src/infrastructure/database/drizzle/repositories/drizzle-row.repository.ts
    apps/api/src/core/use-cases/batch/get-batch.use-case.ts
  ...
  <action>
    **1. Add countByBatchId to RowRepository**

    In `apps/api/src/core/repositories/row.repository.ts`, add abstract method:
    ```typescript
    public abstract countByBatchId(batchId: string): Promise<number>;
    ```

    In `apps/api/src/infrastructure/database/drizzle/repositories/drizzle-row.repository.ts`, implement:
  ...
  <done>
    Three use case files exist with ownership validation, countByBatchId is implemented in both abstract and Drizzle repositories, barrel export updated. TypeScript compiles cleanly.
  </done>
</task>
...
7. countByBatchId exists on both abstract RowRepository and DrizzleRowRepository
...
10. ListBatches computes totalRows per batch via countByBatchId
</verification>
...
- Pagination uses Zod coercion with defaults (50/0) and limits (1-100)
