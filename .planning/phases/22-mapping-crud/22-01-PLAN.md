...
must_haves:
  truths:
    ...
    - "ListMappingsUseCase returns paginated mappings with stepCount and optional URL filter"
  ...
  artifacts:
    ...
    - path: "apps/api/src/core/use-cases/mapping/list-mappings.use-case.ts"
      provides: "ListMappingsUseCase with pagination and URL filter"
  ...
  key_links:
    ...
    - from: "ListMappingsUseCase"
      ...
      via: "paginated query with optional URL filter"
...
<task type="auto">
  <name>Task 1: Extend MappingRepository with pagination and URL filter methods</name>
  <files>
    apps/api/src/core/repositories/mapping.repository.ts
    apps/api/src/infrastructure/database/drizzle/repositories/drizzle-mapping.repository.ts
  </files>
...
1. In `mapping.repository.ts`, add these abstract methods:
   ...
   - `findByProjectIdPaginated(projectId: string, limit: number, offset: number, targetUrl?: string): Promise<{ items: Mapping[]; total: number }>` - Paginated query with optional URL filter
...
2. In `drizzle-mapping.repository.ts`, implement:
   ...
   **findByProjectIdPaginated**:
   - Build conditions: `eq(mappings.projectId, projectId)` AND `isNull(mappings.deletedAt)`
...
3. Import `desc, sql, count` from drizzle-orm as needed.
  </action>
  <verify>
    TypeScript compiles: `cd apps/api && npx tsc --noEmit`
  </verify>
  <done>
    MappingRepository has findByIdWithDeleted, findByProjectIdPaginated, countStepsByMappingId methods implemented with soft-delete filtering and inverted URL prefix matching
  </done>
</task>
...
**Update `apps/api/src/core/use-cases/index.ts`:** Add `export * from './mapping';`
  ...
  <done>
    5 use cases exist with proper ownership validation, 404/403 separation, defense-in-depth, and correct return types
  </done>
</task>
...
3. Repository methods support pagination and URL filter
4. Ownership validation follows established 404/403 pattern from ListBatchesUseCase
</verification>

<success_criteria>
- MappingRepository has findByIdWithDeleted, findByProjectIdPaginated, countStepsByMappingId
