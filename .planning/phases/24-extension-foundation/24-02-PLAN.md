...
must_haves:
  ...
  key_links:
    - from: "apps/extension/src/storage/index.ts"
      to: "wxt/storage"
      via: "import"
...
<task type="auto">
  <name>Task 3: Create storage module index and integrate with background</name>
  <files>
    apps/extension/src/storage/index.ts
    apps/extension/entrypoints/background.ts
  </files>
  <action>
1. Create `apps/extension/src/storage/index.ts`:
```typescript
/**
 * Storage Module
 *
 * Provides type-safe storage accessors for the extension.
 * Uses WXT's storage API over chrome.storage.local.
 *
 * Usage:
...
2. Update `apps/extension/entrypoints/background.ts` to use storage:
```typescript
...
export default defineBackground(() => {
  ...
  initializeStorage().catch((error) => {
    console.error('[Populatte] Storage initialization failed:', error);
  });
  ...
  browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message.type === 'GET_AUTH') {
      return storage.auth.getAuth().then((auth) => ({
        success: true,
        data: auth,
      }));
    }
    ...
    if (message.type === 'GET_SELECTION') {
      return storage.selection.getSelection().then((selection) => ({
        success: true,
        data: selection,
      }));
    }
...
```
  ...
  <verify>
    `cat apps/extension/src/storage/index.ts | grep "export const storage"` and `cat apps/extension/entrypoints/background.ts | grep "initializeStorage"`
  </verify>
  <done>Storage module exports unified storage object and background initializes storage on startup</done>
</task>
...
5. `apps/extension/src/storage/index.ts` exports unified storage object
6. `apps/extension/entrypoints/background.ts` imports and initializes storage
7. Extension still builds: `cd apps/extension && npm run build`
</verification>

<success_criteria>
- Storage types defined with AuthState, SelectionState, PreferencesState
- Type-safe accessors created for each storage domain
- Storage uses WXT's defineItem with fallback defaults
- Background service worker initializes storage on startup
