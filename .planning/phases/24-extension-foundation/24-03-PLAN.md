...
must_haves:
  ...
  artifacts:
    ...
    - path: "apps/extension/entrypoints/background.ts"
      provides: "Complete service worker with storage + messaging"
      contains: ["initializeStorage", "createMessageRouter"]
  key_links:
    ...
    - from: "apps/extension/entrypoints/background.ts"
      ...
      via: "import handlers"
      pattern: "createMessageRouter"
    - from: "apps/extension/entrypoints/background.ts"
      to: "apps/extension/src/storage/index.ts"
      via: "import storage"
...
Output: A messaging module with discriminated union message types, typed send functions, and a handler registry pattern. This plan also creates the FINAL complete background.ts that integrates both storage (from 24-02) and messaging.

**Important:** This plan REPLACES the minimal background.ts created in 24-01 with the full implementation that includes storage initialization AND message routing.
</objective>
...
<task type="auto">
  <name>Task 3: Wire up messaging in all contexts (complete background.ts)</name>
  <files>
    apps/extension/entrypoints/background.ts
    apps/extension/entrypoints/content.ts
    apps/extension/entrypoints/popup/App.tsx
...
**IMPORTANT:** This task REPLACES the minimal background.ts from Plan 24-01 with the complete version that includes BOTH storage initialization (from 24-02) AND message routing. This is the FINAL background.ts for this phase.

1. **REPLACE** `apps/extension/entrypoints/background.ts` with the complete implementation:
```typescript
...
```
  ...
  <verify>
    `cat apps/extension/entrypoints/background.ts | grep "createMessageRouter"` and `cat apps/extension/entrypoints/background.ts | grep "initializeStorage"` and `cat apps/extension/entrypoints/popup/App.tsx | grep "sendToBackground"` and `cd apps/extension && npm run build`
  </verify>
  <done>All contexts wired with messaging system, background.ts is complete with storage + messaging integration</done>
</task>
...
5. `apps/extension/entrypoints/background.ts` uses BOTH createMessageRouter AND initializeStorage
...
- Background initializes storage on startup
- Content script responds to messages
- Extension builds successfully with complete messaging and storage integration
</success_criteria>
