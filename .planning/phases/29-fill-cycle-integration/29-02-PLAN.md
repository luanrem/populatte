...
must_haves:
  truths:
    ...
    - "Fill button onClick sends FILL_START message to background"
  artifacts:
  ...
  key_links:
    - from: "apps/extension/entrypoints/popup/components/FillControls.tsx"
      ...
      via: "onFill prop calls handleFill which sends FILL_START"
      pattern: "sendToBackground.*FILL_START"
...
<task type="auto">
  <name>Task 3: Wire progress updates and verify fill button flow</name>
  <files>
    apps/extension/entrypoints/background.ts
  </files>
  <action>
Ensure fill progress flows through to popup and verify the complete chain:
...
4. Verify end-to-end fill button flow:
   - FillControls.tsx: onClick={onFill} prop
   - App.tsx: handleFill() calls sendToBackground({ type: 'FILL_START' })
   - background.ts: FILL_START handler executes fill orchestration

   Key verification: The existing handleFill() in App.tsx already sends FILL_START:
   ```typescript
   ...
   ```
   This is passed to FillControls as onFill prop, which calls it on button click.
...
6. Handle edge cases in FILL_START:
...
Just ensure background properly updates these.
  ...
  <verify>
Manual test (critical - verifies FILL_START message flow):
1. Load extension, connect, select project with mapping, select batch
2. Navigate to mapped URL
3. Open browser DevTools > Console
4. Click Fill button in popup
5. Verify console shows: "[Background] Received message: FILL_START"
6. Verify popup shows progress (filling -> success/failed)
7. Verify form fields are populated on the page
  </verify>
  <done>
Fill flow works end-to-end: Fill button click -> FILL_START message -> progress update -> step execution -> result display
  </done>
</task>
...
3. Manual verification:
   ...
   - Click Fill button
   - Verify:
     a. Console shows "[Background] Received message: FILL_START" (confirms button->message flow)
     b. Button shows "Filling..." state
     c. Form fields are populated with row data
     d. Button returns to normal state after completion
     e. FILL_PROGRESS message received (check console)
</verification>
...
- Fill button onClick properly sends FILL_START message (key link verified)
