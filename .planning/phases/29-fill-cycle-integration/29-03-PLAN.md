...
must_haves:
  truths:
    ...
    - "User can retry a row by navigating back and filling again"
...
3. Manual test:
  ...
  <done>
Row status automatically updates to VALID on success, ERROR on failure, with error details stored
  </done>
</task>
...
- FILL-07 requirement satisfied: Error state with reason stored in database
- FILL-08 requirement satisfied: User can manually advance (row status persists)
- BROW-03 requirement validated: errorMessage field stores failure reason
- Complete flow: Fill -> status update -> Next -> repeat
</success_criteria>
