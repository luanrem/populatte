...
function UrlChip({ className, count, ...props }: UrlChipProps) {
  ...
  return (
    <span
      data-slot="url-chip"
      className={cn(
        "inline-flex w-fit shrink-0 items-center gap-1 rounded-full border border-border bg-mocha-50 px-2 py-0.5 text-[0.6875rem] font-medium text-muted-foreground whitespace-nowrap [&>svg]:size-3 [&>svg]:pointer-events-none",
        className
      )}
      {...props}
    >
